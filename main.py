...
def dnd_question_getter(s, identifier):
    pattern = r'<simpleAssociableChoice[^>]+identifier="{}"[^>]+><strong>(.*?)</strong>.*?</simpleAssociableChoice>'.format(
        identifier)
    match = re.search(pattern, s)
    if match:
        return match.group(1)
...
def main():
    ...
    xml_string = r.replace('\n', '').replace('\\n', '').replace('\\r', '').replace(
        '  ', '').encode().decode('unicode_escape').replace('\'', '').replace(u'\xa0', u' ')
    xml_string = remove_non_ascii(xml_string)
    xml_escaped = html.unescape(xml_string)
    xml_escaped = remove_text_between_tags(
        xml_escaped, '<div id="headerAudio"', '>')
    ...
    with open('corregido.json') as corregido:
        txt_corregido = corregido.read()

    # Para eliminar los saltos de linea "$\n" y seleccionar todos con .* en vscode
    error_txt = open('error.json', 'w')
    error_txt.write(xml_quoted)

    try:
        xml_dict = json.loads(txt_corregido)
    ...
    for xml in xml_dict:
        xml = remove_text_between_tags(xml_dict[xml].replace(
            '\\', ''), '<div id="options">', '</div>')
        ...
        if indication:
            ...
            respuestas.write(f'{indi}: {indication}')
        ...
        if parent:
            ...
            if question:
                ...
                respuestas.write(f'{good} {question}')

        # ---------------------------- Answers ----------------------------
        answers = re.findall(
            r'<correctResponse><value>(.*?)</value>(.*?)</correctResponse>', xml, re.DOTALL)
        ...
        for answer in answers:
            if isDnD:
                ...
                respuestas.write(f'  {alert} {answer[0]}')
